...
const LatestBlogs = () => {
    ...
    const getLatestBlogs = async () => {
        try {
            ...
            const q = query(blogsRef, orderBy("createdAt", "desc"), limit(3)); // Order by 'createdAt' and limit to 3
            const querySnapshot = await getDocs(q);
            const blogs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setLatestBlogs(blogs); // Save the fetched blogs to state
        } catch (error) {
    ...
    return (
        <div className="mx-auto p-4 bg-gray-800 rounded-lg shadow-lg w-[90%] md:w-full  lg:w-[40%]">
            <h2 className="text-xl font-semibold text-center text-white mb-4">Latest Blogs</h2>
