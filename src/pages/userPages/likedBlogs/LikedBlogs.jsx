...
const LikedBlogs = () => {
    const { fetchUserData, userData, blogs, getAllBlog } = useAuth();
            ...
            <div className="flex-1 p-8 md:p-16 bg-primary shadow-lg md:h-screen md:overflow-y-auto">
                ...
                <div className="space-y-6">
                    {blogs.filter(blog => userData?.likedBlogs.includes(blog.id))
                        .map(b => (
                            <div key={b.id} className="p-6 bg-primary rounded-lg border-2 border-secondary shadow-md hover:shadow-xl transition-shadow duration-300 w-[80%] mx-auto">
                                {/* Blog Image */}
                                <img
                                    src={b.imageUrl}
                                    alt={b.title}
