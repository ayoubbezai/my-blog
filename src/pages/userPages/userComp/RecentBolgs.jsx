...
const RecentBlogs = () => {
    ...
    return (
        <>
            ...
            <div className=" py-8">
                <div className="grid grid-rows-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-6 p-6">
                    {blogs.length > 1 ? (
                        <>
                            {/* Left-Side Featured Blog (taking half space) */}
                            <div className="col-span-1 row-span-2 p-4 flex flex-col gap-3 shadow-lg bg-primary rounded-lg">
                                <h2 className="text-lg md:text-2xl mb-2 font-bold leading-snug text-white">
                                    {blogs[0].title}
                                </h2>
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col  ">
                                        <p className="text-sm md:text-base  leading-loose text-white mt-3">
                            ...
                            <div className="col-span-1 row-span-1 p-4 flex flex-row gap-6 shadow-lg bg-primary rounded-lg">
                                {/* Image on the left */}
                                <img
                                    src={blogs[1].imageUrl}
                                    alt={blogs[1].title}
                                    className="w-2/5 h-32 md:h-40 object-cover rounded-lg"
                                />
                                {/* Text on the right */}
                                <div className="flex flex-col gap-3 w-3/5">
                                    <h2 className="text-base md:text-xl font-bold leading-snug text-white">
                                        {blogs[1].title}
                                    </h2>
                                    <p className="text-xs md:text-sm leading-loose text-white mt-2">
                ...
                {blogs.length > 3 && (
                    <div className="grid grid-cols-1 gap-6 p-6">
                        <div className="col-span-1 p-4 flex flex-row gap-20 shadow-lg bg-primary rounded-lg w-full">
                            {/* Image on the left */}
                            <img
                                src={blogs[3].imageUrl}
                                alt={blogs[3].title}
                                className="w-1/3 h-40 md:h-48 object-cover rounded-lg"
                            />
                            {/* Text on the right */}
                            <div className="flex flex-col gap-3 w-2/3">
                                ...
                                <p className="text-sm md:text-base leading-loose w-[90%] text-white mt-3">
